...
try:
    from edmcoverlay_improved import Overlay, ServiceManager, OverlayConnectionError, OverlayServiceError
...
class TestServiceManager(unittest.TestCase):
    ...
    def test_ensure_service_start(self, mock_popen):
        ...
        if hasattr(self, 'service_manager'):
            ...
            with patch.object(self.service_manager, 'find_server_program', return_value='test.exe'):
                with patch.object(self.service_manager, 'check_game_running', return_value=True):
                    with patch.object(self.service_manager, 'is_service_alive', return_value=False):
...
class TestConfig(unittest.TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open, read_data='{"server": {"port": 5015}}')
...
class IntegrationTest(unittest.TestCase):
    ...
    @unittest.skipUnless(lambda self: self.server_available, "Overlay server not available")
