...
class PerformanceMetrics:
    ...
    def record_error(self, error_type: str, error_message: str = ""):
        ...
        with self.lock:
            ...
            logger.warning(f"Error recorded - Type: {error_type}, Message: {error_message}")
    ...
    def get_message_rate(self, window_minutes: int = 1) -> float:
        ...
        with self.lock:
            ...
            time_span = (datetime.utcnow() - recent_messages[0]['timestamp']).total_seconds()
    ...
    def get_average_message_duration(self, message_type: str = None) -> float:
        ...
        with self.lock:
            if message_type:
                durations = [m['duration'] for m in self.message_times 
    ...
    def get_error_rate(self, window_minutes: int = 5) -> float:
        ...
        with self.lock:
            ...
            recent_error_count = sum(self.error_counts.values()) * (len(recent_messages) / max(self.total_messages_sent, 1))
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            connect_events = [c for c in self.connection_times if c['event'] == 'connect']
            disconnect_events = [c for c in self.connection_times if c['event'] == 'disconnect']
            
            avg_connect_time = 0.0
            if connect_events:
                connect_durations = [c['duration'] for c in connect_events if c['duration'] > 0]
                avg_connect_time = sum(connect_durations) / len(connect_durations) if connect_durations else 0.0
    ...
    def _get_memory_usage(self) -> float:
        ...
        try:
            ...
            import sys
            return sys.getsizeof(self.message_times) + sys.getsizeof(self.connection_times) / 1024 / 1024


class PerformanceMonitor:
    """Context manager for monitoring operation performance"""
    
    def __init__(self, metrics: PerformanceMetrics, operation_name: str, operation_type: str = "message"):
